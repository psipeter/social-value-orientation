...
#         unit_ssp = ssp / np.linalg.norm(ssp, axis=1)
    ...
    def get_sample_points(self, samples_per_dim=100, method='length-scale', seed=0):
        '''
...
class HexagonalSSPSpace(SSPSpace):
    ...
    def sample_grid_encoders(self, n, seed=0):
        sample_pts = self.get_sample_points(n, method='sobol', seed=seed)
        rng = np.random.RandomState(seed=seed)
        N = self.num_grids
        if N < n:
            sorts = np.hstack([np.arange(N), rng.randint(0, N - 1, size = n - N)])
        else:
            sorts = np.arange(n)
